"""Side-by-side: K=100 + smart vs K=100 + smart + per-time scale (current best)."""
...
def main():
    out_dir = REPO / "runs_aux/scale_result"
    ...
    for f in meta["frames"]:
        ...
        pred_a = render_hier("hier_K100_smart_3x", gaussians, xyz_canon, R_cam, Tcam,
                              fov_x, FovY, H, W, t, pipe, bg, N)
        pred_b = render_hier("hier_K100_smart_scale", gaussians, xyz_canon, R_cam, Tcam,
                              fov_x, FovY, H, W, t, pipe, bg, N)
        p_a.append(psnr(pred_a, gt_rgb))
        p_b.append(psnr(pred_b, gt_rgb))

        if v == 0:
            sep = np.ones((H, 4, 3))
            row = np.concatenate([gt_rgb, sep, pred_a, sep, pred_b], axis=1)
            ...
            d.text((col_w + W // 2 - 110, 6),
                   f"K=100 smart ({p_a[-1]:.2f} dB)", fill="black", font=font)
            d.text((2 * col_w + W // 2 - 130, 6),
                   f"+ per-time scale ({p_b[-1]:.2f} dB)",
    ...
    print(f"K=100 + smart 3x          : mean PSNR = {np.mean(p_a):.3f}")
    print(f"K=100 + smart + per-time s : mean PSNR = {np.mean(p_b):.3f}")
    print(f"  uplift                  : {np.mean(p_b) - np.mean(p_a):+.3f} dB")
    return 0
