"""Eval LBS part-rigid model against SV4D-internal test set."""
...
def main() -> int:
    ...
    p.add_argument("--partrigid_label", default="lbs_v1")
    ...
    # LBS weights if available, else fall back to hard part_id
    if "arm_weights" in state.files:
        arm_weights = state["arm_weights"]
        print(f"[eval] using LBS weights (mean={arm_weights.mean():.3f})")
    else:
        arm_weights = (state["part_id"] == 0).astype(np.float32)
        print(f"[eval] no LBS weights, using hard part ID")
    ...
    print(f"[eval] {args.scene}: {len(test_meta['frames'])} test frames")
    ...
    print()
    print(f"[eval] === {args.partrigid_label} on {args.scene} ===")
