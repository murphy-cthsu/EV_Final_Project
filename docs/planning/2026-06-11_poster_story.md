...
> 「我們想用影片生成模型當免費的多視角監督來重建 4D。我們的解耦方法比 joint baseline 好 8.6 dB —— 但輸出有點糊。我們本來以為是 bug,查證後發現:糊的地方剛好就是生成模型自己不一致的地方,因為我們凍結了結構,噪音無處可藏。所以我們反過來把它當量測儀器:量出 SV4D 有一個『可靠錐』—— 只有 input view 周圍可信,偏 120° 後保真度掉 18 dB、一半的靜止像素在假動。而且我們的 fit-residual 量法完全不需要 ground truth,跟純幾何的 epipolar 檢驗、跟有 GT 的比對,三個獨立儀器互相印證。」
...
│    (slide1/slide2 drawio)    │ ⑤ 時空+定位+物類(D2+D3)        │
...
- 需要一個 pose 對齊的乾淨 canonical;hellwarrior 顯示 pose 不對齊時重建崩(但 probe 仍有效)。
