...
def _frame_to_rgb_uint8(image: Image.Image) -> np.ndarray:
    if image.mode == "RGBA":
...
def preprocess_video(
    ...
    images_v0 = []
    if not object_crop:
        print(f"object_crop=False: simple resize to {H}x{W} (no bbox crop / padding)")
        for image in images:
            rgb = _frame_to_rgb_uint8(image)
            resized = np.array(
                Image.fromarray(rgb).resize((W, H), Image.LANCZOS), dtype=np.uint8
            )
            images_v0.append(resized)
    else:
        ...
        for image in images:
            ...
            images_v0.append(_frame_to_rgb_uint8(rgba))
